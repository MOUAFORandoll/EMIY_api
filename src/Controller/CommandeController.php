...
class CommandeController extends AbstractController
{
    ...
    /**
     * @Route("/commande/read/all", name="commandeReadAll", methods={"POST"})
     * @param Request $request
     * @return JsonResponse
     * @throws ClientExceptionInterface
     * @throws DecodingExceptionInterface
     ...
     * 
     */
    public function commandeReadAll(Request $request)
    {

        // $typeCompte = $AccountEntityManager->getRepository(TypeCompte::class)->findOneBy(['id' => 1]);
        $data = $request->toArray();
        $possible = false;
        if (empty($data['adminkeySecret'])) {

            return new JsonResponse([
                'message' => 'Veuillez recharger la page et  preciser votre keySecret '
            ], 400);
        }
        $admin = $this->em->getRepository(UserPlateform::class)->findOneBy(['keySecret' => $data['adminkeySecret']]);
        if ($admin) {
                ...
                foreach ($llcom  as $commande) {
                    ...
                    if ($commande) {
                        ...
                        if (!$llcom) {
                            ...
                            if (count($lP) > 0) {
                                $cf = [
                                    'status' => $this->getStatus($commande->getStatusFinish()),
                                    'codeCommande' => 'Com' . $commande->getId(),
                                    'codeCommandeS' =>  $commande->getCodeCommande(),
                                    'date' => date_format($commande->getDateCreated(), 'Y-m-d H:i'),
